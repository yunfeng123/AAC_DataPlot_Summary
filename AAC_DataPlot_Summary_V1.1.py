...
def run_report():
    ...
    pic_name.sort_values(by=[3, 2, 0, 1], axis=1, inplace=True)  # 排序，先按照图类别，然后是组的类别，最后是Config
...
def run():
    global text_info_line
    time_t0 = time.time()
    text_info_line = 2  # 监控text_info已经输入到第几行
    text_info.delete(1.0, END)
    text_info.insert(1.0, 'Start -> Reading CSV File' + '\n')
    text_info.update()
    data_csv, USL, LSL, Overlay, Staiton, project = csv_process.csv_process(filepath_CSV)

    config_name = entry_config.get()
    data_ini = pd.read_excel(filepath_ini, sheet_name=Staiton)
    ...
    for i in range(len(data_ini['Data Item'])):
        ...
        if find_flag:
            ...
            if graph_flag == 'Histogram' and data_ini.loc[i, 'Monitor'] == data_ini.loc[i, 'Monitor']:  # Monitor列判定是否画图
                ...
                print_info, out_his = Histogram.histogram(Staiton, pic_dic, config_name, graph_class, item_name,
                                                          USL_histogram, LSL_histogram, data_csv[item_name],
                                                          data_ini.loc[i, 'Axis Upper Limit'],
                                                          data_ini.loc[i, 'Axis Lower Limit'])
                ...
                if data_ini.loc[i + 1, 'Plot Type'] == data_ini.loc[i + 1, 'Plot Type']:
                    print_info, out_box = Boxplot.Boxplot(Staiton, pic_dic, config_name, graph_class, USL_tem, LSL_tem,
                ...
                print_info = Sweep.Sweep(Staiton, pic_dic, config_name, graph_class, USL_tem, LSL_tem, sweep_data_tem,
    ...
    # 保存数据到Excel Table
    save_table_name = project + '_' + Staiton + f'_Summary Table_{now_date}.xlsx'
...
# 主窗口
root = Tk()
root.title('AAC DataPlot Summary_1.0')
root.resizable(0, 0)
...
entry_config.place(relx=0.01, rely=y_start, relwidth=0.86, relheight=0.06)
lbe_config = Label(root, text='CONFIG')
lbe_config.place(relx=0.88, rely=y_start, relwidth=0.11, relheight=0.06)
