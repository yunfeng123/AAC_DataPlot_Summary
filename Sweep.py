...
def Sweep(Station, pic_dic, config_name, graph_class, USL_List, LSL_List, data_Sweep, Upper_Axis, Lower_Axis, ini_pic_order, N):
    ...
    for i in data_Sweep.columns:
        data_ave.append(data_Sweep[i].mean())
        data_ave_3std_P.append(data_Sweep[i].mean() + 3 * data_Sweep[i].std())
        data_ave_3std_N.append(data_Sweep[i].mean() - 3 * data_Sweep[i].std())

    for i in range(len(data_Sweep)):
        plt.plot(data_Sweep.columns, data_Sweep.iloc[i], color='DeepSkyBlue', lw='0.5')
